...
YEAR_RULES = """
年份規則（非常重要）：
- 文件寫明「2026年」或具體年份，event_date 必須使用該年份
- 學年格式 2025-2026：9月至12月用 2025，1月至8月用 2026
- 只有月日時，參考文件頁首學年、文件內年份及今日日期推斷
- 嚴禁將文件中的 2026 改為 2025
"""

SYSTEM_PROMPT_TEMPLATE = """你是香港幼稚園文件分析助手。從通告中提取活動和日期。
今日日期：{today}（香港）
...
COMPACT_RETRY_PROMPT_TEMPLATE = """只回覆精簡 JSON。summary 不超過 40 字。只列出最重要、有明確日期的活動（最多 15 個）。不可重複內容。年份必須與文件一致，學年2025-2026時1-8月用2026年。
...
def _get_hk_today() -> datetime:
    return datetime.now(HK_TZ)


def _build_system_prompt() -> str:
    today = _get_hk_today().strftime("%Y年%m月%d日")
    return SYSTEM_PROMPT_TEMPLATE.format(today=today, year_rules=YEAR_RULES)


def _build_compact_retry_prompt() -> str:
    return COMPACT_RETRY_PROMPT_TEMPLATE


def _extract_document_years(text: str) -> list[int]:
    years = set()
    for match in re.finditer(r"(20[2-3]\d)", text):
        years.add(int(match.group(1)))
    return sorted(years)


def _extract_academic_year(text: str) -> tuple[int, int] | None:
...
def _build_date_context(source_text: str) -> str:
    today = _get_hk_today()
    lines = [f"今日：{today.strftime('%Y年%m月%d日')}（香港）"]
    years = _extract_document_years(source_text)
    if years:
        lines.append(f"文件中出現的年份：{', '.join(str(y) for y in years)}")
    academic = _extract_academic_year(source_text)
    if academic:
        lines.append(
            f"學年：{academic[0]}-{academic[1]}"
            f"（9-12月→{academic[0]}，1-8月→{academic[1]}）"
        )
    return "\n".join(lines)
...
def _year_from_explicit_source(source_text: str, month: int, day: int) -> int | None:
    for year in sorted(_extract_document_years(source_text), reverse=True):
        patterns = [
            rf"{year}年\s*{month}\s*月\s*{day}\s*日",
            rf"{year}年\s*{month}\s*月",
...
def _year_from_academic(month: int, academic: tuple[int, int]) -> int:
    start_year, end_year = academic
    return start_year if month >= 9 else end_year


def _correct_single_date(
    date_str: str | None,
    source_text: str,
    academic: tuple[int, int] | None,
    doc_years: list[int],
    ref_year: int,
) -> str | None:
    if not date_str or str(date_str).strip() in ("", "null"):
        return date_str
    ...
    explicit_year = _year_from_explicit_source(source_text, month, day)
    if explicit_year and explicit_year != year:
        return _format_date(explicit_year, month, day)

    if academic:
        expected_year = _year_from_academic(month, academic)
        if year != expected_year and year in academic:
            return _format_date(expected_year, month, day)

    if doc_years:
        max_year = max(doc_years)
        if year == max_year - 1 and max_year in doc_years and max_year >= ref_year:
            if str(max_year) in source_text:
                if academic:
                    if _year_from_academic(month, academic) == max_year:
                        return _format_date(max_year, month, day)
                elif month <= 8 or max_year == ref_year:
                    return _format_date(max_year, month, day)

    return date_str


def _correct_event_years(events: list[dict], source_text: str) -> list[dict]:
    if not source_text or not events:
        return events

    academic = _extract_academic_year(source_text)
    doc_years = _extract_document_years(source_text)
    ref_year = _get_hk_today().year
    corrected = []

    for event in events:
        updated = dict(event)
        updated["event_date"] = _correct_single_date(
            updated.get("event_date"),
            source_text,
            academic,
            doc_years,
            ref_year,
        ...
        if updated.get("end_date"):
            updated["end_date"] = _correct_single_date(
...
def _finalize_result(result: dict, source_text: str) -> dict:
    finalized = dict(result)
    finalized["events"] = _correct_event_years(finalized.get("events", []), source_text)
    return finalized
...
async def _analyze_messages(messages: list, models: list[str], retry_text: str = "") -> dict:
    ...
    retry_messages = [
        {"role": "system", "content": _build_compact_retry_prompt()},
...
async def analyze_with_vision(images: list[dict], text_hint: str = "") -> dict:
    ...
    date_context = _build_date_context(text_hint)
...
async def analyze_with_text(text: str) -> dict:
    ...
    date_context = _build_date_context(text)

    messages = [
        {"role": "system", "content": _build_system_prompt()},
        {
            "role": "user",
            "content": f"{date_context}\n\n請分析以下幼稚園文件，提取有明確日期的活動：\n\n{text}",
        },
    ]
...
async def analyze_document(content: dict) -> dict:
    ...
    if text and len(text) >= 30:
        try:
            result = await analyze_with_text(text)
            return _finalize_result(result, text)
    ...
    if images:
        result = await analyze_with_vision(images, text_hint=text)
        return _finalize_result(result, text)
